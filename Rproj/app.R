...
# *********************************************************************************
# Must be set before reticulate initializes Python.
Sys.setenv(RETICULATE_PYTHON = "d:/DEV/pypsa-project/.venv/Scripts/python.exe")
library(reticulate)

# Import the Python function from pypsa_model.py.
py_file <- if (file.exists("pypsa_model.py")) "pypsa_model.py" else "Rproj/pypsa_model.py"
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    ...
    leaflet() %>%
      ...
      setView(lng = 22.5000, lat = 39.0000, zoom = 6) %>%
      addAwesomeMarkers(
