...
---
## [CHAPTER 8 - CS 1](chapter8_cs1.ipynb)
---
...
### 8.7 Case study: Security-Constrained Optimal Power Flow (SCOPF)

**Αυτή η ενότητα εισάγει μια θεμελιώδη αλλαγή παραδείγματος (paradigm shift) στον τρόπο λειτουργίας και βελτιστοποίησης των ηλεκτρικών δικτύων, περνώντας από την εκ των υστέρων αντιμετώπιση των κρίσεων στην προληπτική θωράκιση του συστήματος.**

Η Διαφορά Φιλοσοφίας: Αντιδραστικό OPF vs. Προληπτικό SCOPF
- **Η Αντιδραστική Προσέγγιση (Reactive / Classic OPF)**: Μέχρι τώρα, το μοντέλο έβρισκε την οικονομικά βέλτιστη κατανομή (Optimal Power Flow) υποθέτοντας ότι όλα τα στοιχεία του δικτύου είναι διαθέσιμα. Οι δοκιμές αντοχής (stress tests) γίνονταν εκ των υστέρων (after-the-fact). Αν όμως μια κρίσιμη γραμμή πέσει μετά την κατανομή, το σύστημα οδηγείται σε ακραίο και ακριβό redispatch ή σε αναγκαστικές διακοπές ρεύματος.
- **Η Προληπτική Προσέγγιση (Security-Constrained OPF - SCOPF)**: Το SCOPF δεν περιμένει να συμβεί η καταστροφή για να αντιδράσει. Ενσωματώνει εξ αρχής μαθηματικούς περιορισμούς για ένα προκαθορισμένο σύνολο πιθανών βλαβών (contingencies, π.χ. το κριτήριο N-1). Η λύση που παράγει είναι εξαρχής σχεδιασμένη να αντέξει την απώλεια οποιουδήποτε στοιχείου χωρίς να καταρρεύσει η τροφοδοσία.

Μια Πρακτική Αναλογία
- Κλασικό OPF: Ο προγραμματισμός της διαδρομής ενός φορτηγού μεταφορών γίνεται με τη μοναδική παραδοχή ότι όλοι οι δρόμοι θα είναι ανοιχτοί, επιλέγοντας την πιο σύντομη και φθηνή διαδρομή. Αν μια γέφυρα κλείσει ξαφνικά, το φορτηγό εγκλωβίζεται.
- SCOPF: Ο σχεδιαστής προβλέπει το πιθανό κλείσιμο της γέφυρας και επιλέγει μια εναλλακτική διαδρομή από την αρχή — ακόμη κι αν αυτή η διαδρομή κοστίζει ελαφρώς περισσότερο σε καύσιμα.

Η Οικονομική Υποχώρηση (The Cost of Security)
Αυτή η μετακίνηση προς την ανθεκτικότητα εισάγει ένα νέο οικονομικό δεδομένο:
- Η λύση του SCOPF είναι συνήθως ακριβότερη από αυτή του απλού OPF υπό κανονικές συνθήκες.
- Αυτό το επιπλέον κόστος αποτελεί το «ασφάλιστρο» που πληρώνει ο διαχειριστής για να διασφαλίσει ότι το δίκτυο δεν θα καταρρεύσει αν συμβεί ένα απρόοπτο γεγονός.

**Το SCOPF μετατρέπει έτσι την ασφάλεια του δικτύου από ένα πρόβλημα έκτακτης ανάγκης σε μια μόνιμη παράμετρο οικονομικής και τεχνικής βελτιστοποίησης.**


---

### Σύστημα 5 Κόμβων για Ασφάλεια N-1

**Για την πρακτική επίδειξη και κατανόηση των μηχανισμών του SCOPF (Security-Constrained Optimal Power Flow), η συγκεκριμένη μελέτη περίπτωσης επιστρατεύει ένα απλοποιημένο αλλά ιδιαίτερα διδακτικό δίκτυο 5 κόμβων (five-bus system). Το σύστημα αυτό έχει σχεδιαστεί σκόπιμα με τέτοιο τρόπο ώστε να αναδεικνύει τις δυναμικές και τις προκλήσεις της ασφάλειας N-1.**

Η Τοπολογία του Δικτύου (Fig. 8.5)
Η δομή του συστήματος αποτελείται από δύο βασικά αρχιτεκτονικά μέρη:
- Ο Κεντρικός Βρόχος (The Loop): Τέσσερις κύριοι κόμβοι (buses) συνδέονται μεταξύ τους σχηματίζοντας έναν κλειστό δακτύλιο/βρόχο. Κάθε ένας από αυτούς τους τέσσερις κόμβους είναι εξοπλισμένος με μια συμβατική γεννήτρια (conventional generator).
- Η Ακτινική Απόληξη (The Spur): Ένας πέμπτος κόμβος συνδέεται ακτινικά (radial connection) ως «διακλάδωση» πάνω στον κεντρικό βρόχο.

Σκοπός της Σχεδίασης και Κατανομή Φορτίου

Οι καταναλώσεις και τα φορτία (loads) δεν είναι τυχαία τοποθετημένα. Είναι γεωγραφικά διασκορπισμένα σε όλο το σύστημα με τέτοιο τρόπο ώστε:
- Υπό κανονικές συνθήκες: Να δημιουργούν ελεγχόμενη επιχειρησιακή πίεση (operational stress) στο δίκτυο, αναγκάζοντας τις γραμμές να λειτουργούν κοντά στα όριά τους.
- Υπό συνθήκες σφάλματος (Contingency): Να αποκαλύπτουν αμέσως τα τρωτά σημεία του κλασικού OPF. Για παράδειγμα, αν μια γραμμή του κεντρικού βρόχου τεθεί εκτός λειτουργίας, η ροή ισχύος αναγκάζεται να αναδρομολογηθεί από την εναπομείνουσα διαδρομή του δακτυλίου, δοκιμάζοντας τα όρια θερμικής χωρητικότητας των υπόλοιπων γραμμών.

**Αυτό το στυλιζαρισμένο μοντέλο επιτρέπει στο PyPSA να δείξει καθαρά πώς το SCOPF θα προσαρμόσει προληπτικά την παραγωγή των γεννητριών (redispatch), μειώνοντας τη ροή σε κρίσιμες γραμμές, ώστε αν συμβεί οποιοδήποτε μεμονωμένο σφάλμα (N-1), το σύστημα να παραμείνει ασφαλές χωρίς να προκληθεί υπερφόρτωση ή διακοπή ρεύματος.**

<div class="container">
  <img src="imgs/8_6.png" alt="Περιγραφή" style=" display: block;
  margin-left: auto;
  margin-right: auto;
  width: 40%;">
</div>

Πρόκειται για έναν κλασικό δακτύλιο (loop) τεσσάρων κόμβων, πάνω στον οποίο συνδέεται ακτινικά ένας πέμπτος κόμβος.

Ακολουθεί η αναλυτική χαρτογράφηση του δικτύου για να οτικοποιήσετε τη δομή του:
1. Ο Κεντρικός Δακτύλιος (Buses 0, 1, 2, 3)

Οι τέσσερις πρώτοι κόμβοι σχηματίζουν ένα κλειστό κύκλωμα μέσω των εξής γραμμών:

    Line01: Συνδέει τον Κόμβο 0 με τον Κόμβο 1.

    Line12: Συνδέει τον Κόμβο 1 με τον Κόμβο 2.

    Line23: Συνδέει τον Κόμβο 2 με τον Κόμβο 3.

    Line30: Συνδέει τον Κόμβο 3 με τον Κόμβο 0.

2. Η Ακτινική Απόληξη (Bus 4)

    Ο Κόμβος 4 (Bus 1) δεν κλείνει κάποιο κύκλωμα. Συνδέεται αποκλειστικά πάνω στον Κόμβο 1 μέσω της γραμμής Line14. Αυτό σημαίνει ότι αν η Line14 πάθει βλάβη, ο Κόμβος 4 απομονώνεται πλήρως (blackout), καθώς δεν υπάρχει εναλλακτική διαδρομή (εδώ φαίνεται η ιδιαιτερότητα των ακτινικών γραμμών στην ασφάλεια N-1).


| Κόμβος (Bus) | Παραγωγή (Generator) | Κατανάλωση (Load) | Ρόλος / Σύνδεση στο Δίκτυο |
| :--- | :--- | :--- | :--- |
| **Bus 0** | Γεννήτρια G0 | - | Μέρος του δακτυλίου (Συνδέει Bus 3 και Bus 1) |
| **Bus 1** | Γεννήτρια G1 | Load 1 | **Κεντρικός Κόμβος** (Συνδέει Bus 0, Bus 2 και την ακτινική Line14) |
| **Bus 2** | Γεννήτρια G2 | Load 2 | Μέρος του δακτυλίου (Συνδέει Bus 1 και Bus 3) |
| **Bus 3** | Γεννήτρια G3 | Load 3 | Μέρος του δακτυλίου (Συνδέει Bus 2 και Bus 0) |
| **Bus 4** | - | Load 4 | **Ακτινικός Κόμβος** (Τροφοδοτείται αποκλειστικά από το Bus 1) |

### [CHAPTER 8 - CS 2](chapter8_cs2.ipynb)